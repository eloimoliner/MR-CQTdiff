...
def compress_dB_norm(X, range_val = 70):
    """
    Compresses the input tensor X in dB and normalizes it.

    Args:
        X (numpy.ndarray): Input tensor.
        range_val (float): Range for normalization.

    Returns:
        numpy.ndarray: Compressed and normalized tensor.
    """
    ...
    X_comp = X / np.max(X)
    X_comp = 10 * np.log10(X_comp) * (1 / range_val) + 1
    X_comp[X_comp < 0] = 0
    return X_comp