...
class CQT_nsgt():
    def __init__(self, numocts, binsoct,  M=None, fmax=None, mode="critical", window="hann", flex_Q=None, fs=44100, audio_len=44100, device="cpu", dtype=torch.float32):
        """
            args:
                numocts (int) number of octaves
                binsoct (int) number of bins per octave. Can be a list if mode="flex_oct"
                M (int) number of time frames per frequency bin. Can be a list.
                fmax (float) maximum frequency. If None, the maximum frequency is Nyquist
        ...
        if mode=="matrix" or mode=="matrix_complete" or mode=="matrix_slow":
            ...
            if M is not None:
                #force the M to be the same for all the bins with value forced_M
                self.M[:] = M
            ...
            #round to all the lengths of an octave to the next power of 2
            self.size_per_oct=[]
            idx=1
        ...
        #self.wins = calcwinrange(self.g, rfbas, self.Ls, device=self.device)
        ...
        self.maxLg_enc = int(max(int(ceil(float(len(gii))/mii))*mii for mii,gii in zip(self.M[sl], self.g[sl])))
        ...
        def get_ragged_giis(g, wins, ms, mode):
            ...
            if mode=="oct":
                ...
                ix.append(torch.zeros((1,int(ms[0])),dtype=torch.int64,device=self.device))
                count=0
                for i in range(1,len(g)-1):
                    if count==0 or ms[i] == ms[i-1]:
                        count+=1
                    else:
                        ix.append(torch.zeros((count,int(ms[i-1])),dtype=torch.int64,device=self.device))
                        count=1

                ix.append(torch.zeros((count,int(ms[i-1])), dtype=torch.int64, device=self.device))

                ix.append(torch.zeros((1,int(ms[-1])), dtype=torch.int64, device=self.device))
