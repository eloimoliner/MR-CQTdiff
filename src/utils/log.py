...
def plot_spectrogram(X, refr=None, cplx=True, freqs=None, time=None):
    X=X.squeeze(1)
    X=X.cpu().numpy()
    if cplx:
    ...
    for i in range(X.shape[0]): #iterate over batch size, shity way of ploting all the batched spectrograms
        ...
        if i==0:
             ...
             res=np.concatenate((res,o), axis=1)
    #freqs ia a tensor of shape (F,)
    #time is a tensor of shape (T,)
    #add the frequency and time axis correctly
...
def plot_rir(rir, T_separation, fs, path=None, name=None, save=True):
    '''
    Plot the RIR and indicate the separation time between early and late reflections
    '''
    rir = rir.cpu()
    RIR = torch.stft(rir, n_fft=512, hop_length=128, window=torch.hann_window(512).to(rir.device), return_complex=True)
    
    fig, ax = plt.subplots(1, 2)

    ax[0].pcolormesh(
        torch.linspace(0, RIR.size(-1)/128, RIR.size(-1)), 
        torch.linspace(0, fs/2, RIR.size(-2)),
        20*torch.log10(RIR.abs() + 1e-10))

    ax[1].plot(
        torch.linspace(0, rir.size(-1)/fs, rir.size(-1)), 
        rir)
...
def create_mag_phase_plot():
    ...
    # Update layout
    fig.update_layout(height=600, width=800, title_text="Magnitude and Phase of Filter")
    ...
    # Set y-axes titles
    fig.update_yaxes(title_text="<b>Magnitude [dB]</b>", secondary_y=False)
    fig.update_yaxes(title_text="<b>Phase [rad]</b>", secondary_y=True)
    return fig
