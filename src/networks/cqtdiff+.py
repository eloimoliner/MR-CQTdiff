...
class BiasFreeGroupNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        #C = int(C) if isinstance(C, torch.SymInt) else C
...
class CQTDiffPlus(nn.Module):
    ...
    def __init__(self,
        ...
        """
        super(CQTDiffPlus, self).__init__()
        ...
        if self.representation=="waveform":
            ...
            if multiple_audio_lengths is not None:
                ...
                for audio_len in multiple_audio_lengths:
                    self.CQTransform_dict[audio_len]=CQT_nsgt(cqt.num_octs, cqt.bins_per_oct, mode="oct",window=win,fs=sample_rate, audio_len=audio_len, dtype=torch.float32, device=self.device)
            else: 
        ...
        if self.representation=="waveform":
            ...
            if self.multiple_audio_lengths is None:  
                if self.CQTransform.Ls != inputs.shape[-1]:
                    CQTransform=CQT_nsgt(self.num_octs, self.bins_per_oct, mode="oct",window=self.win,fs=self.sample_rate,
                                          audio_len=inputs.shape[-1], dtype=torch.float32, device=self.device)
                else:
            ...
            X_list=inputs
            X_list=[X_list[i].to(self.device) for i in range(len(X_list))]
